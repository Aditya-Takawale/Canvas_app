...
const StableCanvasFixed: React.FC<StableCanvasProps> = ({ 
  roomId, 
  width = 800, 
  height = 400 
}) => {
  ...
  const saveCanvasState = () => {
    if (!fabricCanvasRef.current || isRedoing) {
      console.log('⏭️ StableCanvas: Skipping save - canvas not ready or redoing');
      return;
    }
  ...
  const handleUndo = () => {
    ...
    if (!fabricCanvasRef.current || historyIndex <= 0) {
      console.log('❌ StableCanvas: Cannot undo - at beginning');
      return;
    }
  ...
  const handleRedo = () => {
    ...
    if (!fabricCanvasRef.current || historyIndex >= canvasHistory.length - 1) {
      console.log('❌ StableCanvas: Cannot redo - at end');
      return;
    }
    ...
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', backgroundColor: '#f5f5f5' }}>
      ...
      <div style={{ backgroundColor: 'white', borderBottom: '1px solid #e0e0e0', padding: '12px 16px' }}>
        <h3 style={{ fontSize: '18px', fontWeight: 'bold', color: '#2d3748', margin: 0 }}>
          🎨 Fixed Stable Canvas - Room {roomId}
        </h3>
...
export default StableCanvasFixed;