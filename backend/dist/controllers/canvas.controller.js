...
exports.getCanvasHistory = exports.saveCanvasState = exports.updateCanvas = exports.getCanvasById = void 0;
...
const getCanvasById = async (req, res) => {
    try {
        const { id } = req.params;
        const canvas = await prisma_1.default.canvas.findUnique({
        ...
        logger_1.default.error('Get canvas error:', error);
        res.status(500).json({
            status: 'error',
...
const updateCanvas = async (req, res) => {
    try {
        ...
        // Update canvas properties
        const updatedCanvas = await prisma_1.default.canvas.update({
            where: { id: parseInt(id) },
            data: {
...
const saveCanvasState = async (req, res) => {
    try {
        ...
        const { state, operations } = req.body;
        // Find canvas
        const canvas = await prisma_1.default.canvas.findUnique({
            where: { id: parseInt(id) },
        });
 ...
 */
const getCanvasHistory = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const operations = await prisma_1.default.drawingOperation.findMany({
        ...
        logger_1.default.error('Get canvas history error:', error);
        res.status(500).json({
            status: 'error',
...
exports.getCanvasHistory = getCanvasHistory;
